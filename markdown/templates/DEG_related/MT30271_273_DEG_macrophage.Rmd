...
### Macrophage & Responder & CTL vs TAK
...
# Find DEGs
DEG.macrophage= DEG.function(obj.srt,
                             filter1 = filter1_selection,
                             groups = selected_groups)
...
plot_volcano_text(mks = DEG.macrophage, title=paste0(filter1_selection, ' ',
                                                     filter2_selection, ' ',
...
#### Up-regulated genes
```{r}
data.frame("UP-regulated genes" = (DEG.macrophage %>% 
                                                 filter(DE == 'UP') %>% 
                                                 select(gene) %>% 
                                                 pull() %>% 
                                                 paste0(collapse = ',')),
           row.names = 'macrophage') %>% DT::datatable(options = list(scrollX=T))
```
...
```{r}
data.frame("DN-regulated genes" = (DEG.macrophage %>% 
                                                 filter(DE == 'DN') %>% 
                                                 select(gene) %>% 
                                                 pull() %>% 
                                                 paste0(collapse = ',')),
...
### Macrophage & Responder & CTL vs COM
...
### Macrophage & Non Responder & CTL vs TAK
...
### Macrophage & Non Responder & CTL vs COM
...
data.frame("UP-regulated genes" = (DEG.out %>% 
                                     ...
                                     pull() %>% 
                                     paste0(collapse = ',')),
           row.names = 'T cell') %>% DT::datatable(options = list(scrollX=T))
```
...
### T cell & Non Responder & CTL vs TAK
...
### T cell & Non Responder & CTL vs COM
