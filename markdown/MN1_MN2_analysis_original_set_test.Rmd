---
title: "NK project: MN1 vs MN2, original set retest"
output: html_document
---
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo=T, fig.align = "center", message=F, warning=F, fig.height = 5, fig.width = 6)
```
## R Markdown
...
obj.srt = readRDS(paste0(dir,'NK_Marco.MN1_8.umi.mt.filtered.mn1.2.annotated.23.01.25.rds'))
```
...
obj.srt@meta.data[,c('orig.ident','inclusive.celltype')] %>% table() %>% data.frame() %>% tidyr::spread(inclusive.celltype, Freq)
```
...
DimPlot(obj.srt, group.by = 'celltype.230125', label = F, label.box = T, label.size = 3) + theme(plot.title = element_blank()) 
DimPlot(obj.srt, group.by = 'celltype.230125', label = T, label.box = T, label.size = 3) + theme(plot.title = element_blank()) 
DimPlot(obj.srt, group.by = 'inclusive.celltype', label = T, label.box = T, label.size = 3) + theme(plot.title = element_blank()) 
DimPlot(obj.srt, group.by = 'inclusive.celltype', label = F, label.box = T, label.size = 3) + theme(plot.title = element_blank()) 
...
# Import mks files
dir='~/Desktop/DF/DFCI_Barbie/DFCI_Barbie_NK_Marco/data/mks/MN1.MN2/DE/'
mks.nk= read.csv(paste0(dir,'NK_Marco.MN1.2.NK.ctc.cgc.DE.23.01.30.csv'), row.names = 1)
mks.huvec=read.csv(paste0(dir,'NK_Marco.MN1.2.HUVEC.ctc.cgc.DE.23.01.31.csv'), row.names = 1)
mks.fibro=read.csv(paste0(dir,'NK_Marco.MN1.2.fibroblast.ctc.cgc.DE.23.01.30.csv'), row.names = 1)
mks.corl=read.csv(paste0(dir,'NK_Marco.MN1.2.CORL47.ctc.cgc.DE.23.01.31.csv'), row.names = 1)
...
plot_vln_by_compare <- function(main_cell_type, seurat_obj, gene_vector) {
  seurat_obj@meta.data <- seurat_obj@meta.data %>%
    mutate(compare = ifelse(inclusive.celltype == main_cell_type & orig.ident == 'MN1', paste0(main_cell_type, ' MN1'),
                            ifelse(inclusive.celltype == main_cell_type & orig.ident == 'MN2', paste0(main_cell_type, ' MN2'), 'other')))
...
cell_types <- c('CORL47', 'HUVEC', 'Fibro', 'NK')
...
dir='~/Desktop/DF/DFCI_Barbie/DFCI_Barbie_NK_Marco/data/mks/MN1.MN2/DE/'
DEG.combined= read.csv(paste0(dir, 'NK_Marco.MN1.2.combined.mks.23.08.16.csv'), row.names = 1)
...
```{r}
dir="~/Desktop/DF/DFCI_Barbie/DFCI_Barbie_NK_Marco/data/mks/MN1.MN2/DE/"
gsea.all = read.csv(paste0(dir,'GSEA.MN1.MN2.original.all.23.08.16.csv'), row.names = 1)
