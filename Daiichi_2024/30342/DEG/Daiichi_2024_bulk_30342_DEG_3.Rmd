...
res6 = add_DE_information(res = res6)
...
plot_DE_information(res6, fc = 1.5, pval = 0.05, p_col = 'padj')
plot_DE_information(res6, fc = 1.5, pval = 0.05, p_col = 'pvalue')

```

#### Volcano plot (version3)
```{r, fig.width=8, fig.height=5}
p1 = plot_DE_information(res6, fc = 1.5, pval = 0.05, p_col = 'padj')

p2 = plot_DE_information(res6, fc = 1.5, pval = 0.05, p_col = 'pvalue')
...
#### DEG table
```{r}
res6 %>% DT::datatable(extensions = "Buttons", options = list(dom="Bfrtip", buttons=c("csv","excel"), pageLength=10))
```
...
# Application 
gsea.res6 = perform_GSEA(res = res6, ref = hallmark) 

filtered_gsea6 = gsea.res6 %>% mutate(sig= ifelse(pvalue <= 0.05,"p value <= 0.05", "p value > 0.05"))
```
...
gsea_nes_plot_pval(filtered_gsea6, title = t, fontsize.x = 8, fontsize.y = 8)
```
...
gsea_nes_plot(filtered_gsea6, title = t, fontsize.x = 8, fontsize.y = 8)
```



```{r}
res6 %>% write.csv(paste0(dir,"data/30342/res6.24.06.25.csv"))
```
