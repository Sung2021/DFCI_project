--- 
title: "MT30217 MT30273 comparison analysis"
...
obj.srt@meta.data %>% select(RNA_snn_res.0.2, res) %>% table() %>% data.frame() %>% tidyr::spread(res, Freq) %>% DT::datatable(caption = 'Number of Cells by clusters and R/NR')
obj.srt@meta.data %>% select(RNA_snn_res.0.2, res, sample) %>% table() %>% data.frame() %>% tidyr::spread(sample, Freq) %>% DT::datatable(caption = 'Number of Cells by clusters and R/NR and treatment')
```
...
DT::datatable(data, caption = 'Cell type guess based on Vlnplot') 
...
obj.srt@reductions$umap@cell.embeddings %>% data.frame() %>% 
  ...
  facet_wrap(.~obj.srt$sample, ncol = 3) +
...
```{r}
obj.srt@meta.data %>% select(cell_type, res, sample) %>% table() %>% data.frame() %>% tidyr::spread(sample, Freq) %>% DT::datatable(caption = 'Number of Cells by main cell type and R/NR and treatment')
```
