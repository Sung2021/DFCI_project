...
# DEG between MN1 vs MN2 based on "ANNOTATION" from the merged dataset(MN1~MN4)  
...
mks.function= function(obj.srt, celltype='NK'){
  obj.srt@meta.data =obj.srt@meta.data %>% 
    mutate(compare=ifelse(annotation == celltype & orig.ident == 'MN1','g1',
                          ifelse(annotation == celltype & orig.ident == 'MN2','g2','other')))
...
lapply(names(l), function(name) {
  write.csv(l[[name]], paste0(dir, name, '.MN1.2.deg.csv'), row.names = T)
})
...
Please provide your gene list for violin plots.
...
```{r, fig.width=10, fig.height=6, out.width="120%", eval=FALSE}
t= "Monocyte"
...
## Volcanoplot with same axis-scale    
...
### UP-regulated genes of each cell types  
```{r}
print(ven_list)
```
...
### DOWN-regulated genes of each cell types  
```{r}
print(ven_list)
```
...
mks.all = rbind(mks.nk,
                ...
                mks.corl)
dir= '~/Desktop/DF/DFCI_Barbie/DFCI_Barbie_NK_Marco/'
output_file = paste0(dir,"data/UMI500_50000/MN1_MN2_DEG_all.csv")
...
gsea.all %>% write.csv(paste0(dir,"data/UMI500_50000/MN1_MN2_GESA.HALLMARK.all.csv"))

```

## Enrichment in HALLMARK pathways   
   

```{r}
# read csv 
dir= '~/Desktop/DF/DFCI_Barbie/DFCI_Barbie_NK_Marco/'
file_path <- paste0(dir, "data/UMI500_50000/MN1_MN2_GESA.HALLMARK.all.csv")
