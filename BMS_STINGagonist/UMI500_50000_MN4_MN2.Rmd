---
title: "NK project: UMI 500~ 50000 MN4,MN2"
...
This is the initial analysis report of of MN4,MN2 merged dataset.
...
obj.srt = subset(obj.srt, orig.ident %in% paste0("MN", c(4,2)))
...
obj.srt %>% saveRDS(paste0(dir,('rds/MN4_MN2_subset.500.50000.25.singlet.23.10.05.rds')))
```

# Import MN4,MN2 merged data  

```{r}
dir= '~/Desktop/DF/DFCI_Barbie/DFCI_Barbie_NK_Marco/'
## import data
obj.srt = readRDS(paste0(dir,('rds/MN4_MN2_subset.500.50000.25.singlet.23.10.05.rds')))
```
...
color_sample_in_celltype = function(obj.srt, celltype){
  ...
  rows1= obj.srt@meta.data %>% filter(annotation == celltype & orig.ident==paste0("MN",4)) %>% rownames()
  rows2= obj.srt@meta.data %>% filter(annotation == celltype & orig.ident==paste0("MN",2)) %>% rownames()
  obj.srt@meta.data[rows1,]$color= paste0("MN",4)
...
color_sample_in_celltype_Monocytes = function(obj.srt, celltype){
  obj.srt@meta.data$color = "other"
  rows1= obj.srt@meta.data %>% filter(annotation == celltype & orig.ident==paste0("MN",4)) %>% rownames()
  rows2= obj.srt@meta.data %>% filter(annotation == celltype & orig.ident==paste0("MN",2)) %>% rownames()
  obj.srt@meta.data[rows1,]$color= paste0("MN",4)
  ...
  umap= obj.srt@reductions$umap@cell.embeddings %>% data.frame()
  p=umap %>% ggplot(aes(UMAP_1, UMAP_2, color=obj.srt@meta.data$color)) + geom_point(size=0.1) + 
    scale_color_manual(values = c("blue","#F2F4F4")) +theme_classic() + 
    theme(legend.title = element_blank()) +
    guides(colour = guide_legend(override.aes = list(size=3))) +ggtitle(celltype)
  print(p)
}
...
color_sample_in_celltype_Monocytes(obj.srt, celltype = cell_types[5])
...
for (resolution_number in resolution_values) {
  ...
  output_file = paste0(dir,"data/UMI500_50000/MN4_MN2_", res, ".markers.csv")
...
mks = read.csv(paste0(dir,"data/UMI500_50000/MN4_MN2_", res, ".markers.csv"), row.names = 1)
...
DoHeatmap(obj.srt, features = genes$gene, group.by = as.character(res)) +theme(axis.text.y = element_text(size = 7))
}
```
